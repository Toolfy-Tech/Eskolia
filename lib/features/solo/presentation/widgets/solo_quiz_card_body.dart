...
class SoloQuizCardBody extends ConsumerStatefulWidget {
  const SoloQuizCardBody({super.key, this.isExpandedOverride});
  final bool? isExpandedOverride;

  @override
  ConsumerState<SoloQuizCardBody> createState() => _SoloQuizCardBodyState();
}

class _SoloQuizCardBodyState extends ConsumerState<SoloQuizCardBody> {
  ...
  Widget _buildThemeCheckboxRow({
    ...
    bool isBold = false,
    Color activeColor = EskoliaTokens.cyan,
  }) {
    return InkWell(
  ...
  Future<void> _launchQuiz() async {
    ...
    try {
      ...
      final session = QuizSession(
        ...
        title: _selectedPools.length > 1 ? 'Quiz Mixte' : 'Quiz Personnalisé',
        questions: limitedQuestions,
        currentIndex: 0,
        userScores: List<double?>.filled(limitedQuestions.length, null),
  ...
  Widget build(BuildContext context) {
    ...
    final isCollapsed = widget.isExpandedOverride != null
        ...
        : (settingsMap['feature:solo_quiz']?.isCollapsed ?? false);
